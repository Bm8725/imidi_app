...
// NOU: client TUS pt upload rezumabil, cu progres real per-chunk
import * as tus from "tus-js-client";

const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL!;
const SUPABASE_ANON_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!;

// ---- helpers de formatare timp/viteza (folosite si la upload si la download) ----
...
export default function CloudWorkspacePage() {
  ...
  // ---- NOU: progres real de upload (TUS) ----
  const [uploadStatus, setUploadStatus] = useState<TransferStatus | null>(null);

  // ---- NOU: progres real de download (fetch + reader) ----
  const [downloadStatus, setDownloadStatus] = useState<TransferStatus | null>(null);
  ...
  // ---- NOU: upload cu TUS — progres real, viteza, ETA, retry automat + resume ----
  const uploadFileWithTus = (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const upload = new tus.Upload(file, {
        ...
        // Supabase Storage impune chunk fix de 6MB (ultimul poate fi mai mic)
        chunkSize: 6 * 1024 * 1024,
        uploadDataDuringCreation: true,
        metadata: {
          bucketName: "cloud-db-bucket",
          objectName: path,
        ...
        onShouldRetry: (err) => {
          ...
          // nu reincerca la erori de auth/business logic (4xx), doar retea/5xx/conflict
          if (status && status >= 400 && status < 500 && status !== 409) return false;
          setUploadStatus((prev) => (prev ? { ...prev, retrying: true } : prev));
          return true;
        },
        ...
        onProgress: (bytesUploaded, bytesTotal) => {
          ...
          const instantSpeed = elapsedSec > 0 ? bytesDelta / elapsedSec : 0;
      ...
      // reia un upload intrerupt anterior pt acelasi fisier, daca exista (stabilitate la net picat)
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
      alert("Sesiune expirată, te rog reloghează-te.");
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < fileList.length; i++) {
        ...
        try {
          ...
          await uploadFileWithTus(file, path, session.access_token, i + 1, fileList.length);
  ...
  // ---- NOU: download cu progres real (fetch + ReadableStream, pe baza unui signed URL) ----
  const handleDownload = async (bank: any) => {
    ...
    try {
      // 1. generam un signed URL (functioneaza si pe fisiere private, fara sa expunem cheia)
      const { data: signed, error: signErr } = await supabase
        .storage
        .from("cloud-db-bucket")
        .createSignedUrl(bank.storage_path, 120);
      if (signErr || !signed?.signedUrl) throw signErr || new Error("Nu am putut genera link de descărcare.");

      // 2. fetch manual, ca sa putem citi progresul din stream (supabase-js nu expune progres)
      const res = await fetch(signed.signedUrl);
      if (!res.ok || !res.body) throw new Error("Descărcarea a eșuat.");

      const contentLengthHeader = res.headers.get("content-length");
      const totalBytes = contentLengthHeader ? parseInt(contentLengthHeader, 10) : bank.size_mb * 1024 * 1024;
      ...
      while (true) {
        ...
        if (value) {
          ...
          const now = Date.now();
          const elapsedSec = (now - lastTime) / 1000;
          if (elapsedSec > 0.15) { // throttle update-uri, nu la fiecare pachet minuscul
      ...
      setDownloadStatus((prev) => (prev ? { ...prev, percent: 100, etaSec: 0 } : prev));
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 pt-32 pb-12 relative z-10 font-mono tracking-tight bg-[#1C1C1C]/40 border-x border-zinc-900/60 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
        ...
        {error ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch w-full mb-6">
                    ...
                    {uploading ? (
                      <>
                        ...
                        <span className="tracking-normal font-bold">
                          {uploadStatus ? `${Math.round(uploadStatus.percent)}%` : "Syncing..."}
                        </span>
                      </>
                    ) : (
                      <>
            ...
            {/* NOU: card progres UPLOAD — % real, viteza, ETA, stare retry */}
            {uploadStatus && (
              <div className="bg-[#0D0D0D] border border-zinc-800/80 rounded-2xl p-4 mb-6 font-mono text-[10px] text-zinc-400 space-y-2 shadow-[0_8px_25px_rgba(0,0,0,0.4)]">
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-[#FF7A1A]">
                  ...
                  <span className="text-zinc-500">{uploadStatus.index}/{uploadStatus.total}</span>
                </div>

                <div className="flex items-center justify-between">
                  <span className="truncate max-w-[65%] text-white font-bold tracking-tight">{uploadStatus.fileName}</span>
                  <span className="text-[#FF7A1A] font-black">
                    {uploadStatus.retrying ? "Reconectare..." : `${Math.round(uploadStatus.percent)}%`}
                  </span>
                </div>
                  ...
                  <div
                    className={`h-full rounded-full transition-all duration-200 ease-out ${
                      uploadStatus.retrying
                        ...
                        : "bg-gradient-to-r from-[#FF7A1A] to-[#ffa566]"
                    }`}
                ...
                <div className="flex items-center justify-between text-zinc-500 uppercase tracking-wider">
                  <span>{formatSpeed(uploadStatus.speedMbps)}</span>
                  <span>ETA: {formatEta(uploadStatus.etaSec)}</span>
                </div>
              </div>
            )}

            {/* NOU: card progres DOWNLOAD — % real, viteza, ETA */}
            {downloadStatus && (
              <div className="bg-[#0D0D0D] border border-zinc-800/80 rounded-2xl p-4 mb-6 font-mono text-[10px] text-zinc-400 space-y-2 shadow-[0_8px_25px_rgba(0,0,0,0.4)]">
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-cyan-400">
                    <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                ...
                <div className="flex items-center justify-between">
                  <span className="truncate max-w-[65%] text-white font-bold tracking-tight">{downloadStatus.fileName}</span>
                  <span className="text-cyan-400 font-black">{Math.round(downloadStatus.percent)}%</span>
                </div>

                <div className="w-full h-2 bg-zinc-950 rounded-full border border-zinc-800 overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-cyan-600 to-cyan-300 transition-all duration-200 ease-out"
                    style={{ width: `${downloadStatus.percent}%` }}
                  />
                </div>

                <div className="flex items-center justify-between text-zinc-500 uppercase tracking-wider">
                  <span>{formatSpeed(downloadStatus.speedMbps)}</span>
                  <span>ETA: {formatEta(downloadStatus.etaSec)}</span>
                </div>
              </div>
            )}
                    ...
                    <div className="shrink-0 z-20 flex items-center gap-1 font-mono">
                      {delId === b.id ? (
                        ...
                        <>
                          ...
                          {/* Buton Download cu procent live in tooltip/inline */}
                          <button
                            onClick={() => handleDownload(b)}
                            disabled={downloadingId === b.id}
                            className="w-7 h-7 rounded-sm flex items-center justify-center border border-transparent text-zinc-500 hover:text-white hover:bg-[#2C2C2C] hover:border-zinc-700 sm:opacity-0 group-hover:opacity-100 transition-all disabled:opacity-100 cursor-pointer relative"
                            title={downloadingId === b.id && downloadStatus ? `${Math.round(downloadStatus.percent)}% · ${formatSpeed(downloadStatus.speedMbps)}` : "Descarcă"}
                          >
                            {downloadingId === b.id ? (
                              <svg className="w-3.5 h-3.5 animate-spin text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
